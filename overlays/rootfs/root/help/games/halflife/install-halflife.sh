...
if [ -d "$INSTALLDIR/valve/maps" ] || [ -f "$INSTALLDIR/valve/pak0.pak" ]; then
  ...
  echo ""
  echo "How to play:"
  echo "  # Without desktop (KMSDRM, best performance):"
  echo "  su - cubie -c 'kmsdrm-run halflife-kmsdrm'"
  echo ""
  echo "  # Under X11 desktop:"
  echo "  halflife-x11"
  echo "  # Or launch from application menu"
else
  echo "Engine ready. Game data MISSING."
  echo ""
  echo "Next steps — copy valve/ from Steam Half-Life:"
  echo "  # On your PC:"
  echo "  scp -r ~/.local/share/Steam/steamapps/common/Half-Life/valve/ root@<board-ip>:$INSTALLDIR/valve/"
  echo ""
  echo "  # Then on the board:"
  echo "  chown -R cubie:cubie $INSTALLDIR/valve/"
  echo ""
  echo "  # Play:"
  echo "  su - cubie -c 'kmsdrm-run halflife-kmsdrm'"
fi
